...
function tableSortJs(testingTableSortJS = false, domDocumentWindow = document) {
  ...
  function makeEachColumnSortable(th, columnIndex, tableBody, sortableTable) {
    ...
    // sort for responsive table for el with theading-content class
    document.addEventListener("click", function(e){
      if((e.target.classList.contains('theading-content') && e.target.getAttribute('data-theading-index') == columnIndex) || (e.target.closest('.theading-content') !== null && e.target.closest('.theading-content').getAttribute('data-theading-index') == columnIndex)) {
        clickListener()
      }
    });
  }
