...
class RH_WC_Custom_Bundles{
    
    public function __construct() {
        //require RHWCT_DIRPATH .'/public/custom_bundles_tab_content.php';
        $this->init();
        }
        /**
         * Init settings.
    ...
    public function enqueue_scripts() {
        ...
        wp_enqueue_style( 'greenshift_woocommerce_bundle', RHWCT_URIPATH .'assets/css/woo-custom-bundles.css', array(), RHWCT_VERSION, 'all' );
        wp_enqueue_script( 'greenshift_woocommerce_bundle', RHWCT_URIPATH .'assets/js/woo-custom-bundles.js', array( 'jquery' ), RHWCT_VERSION, false );
    ...
    public function add_custom_bundle_panel_data() {
        ...
        <div class="options_group">
            <p class="form-field">
                ...
                <select class="wc-product-search" multiple="multiple" style="width: 50%;" id="custom_bundle_ids" name="custom_bundle_ids[]" 
                data-placeholder="<?php esc_attr_e( 'Search for a product&hellip;', 'greenshiftwoo' ); ?>" 
    ...
    public static function save_custom_bundle_panel_data( $post_id ) {
        $bundle = isset( $_POST['custom_bundle_ids'] ) ? array_map( 'intval', (array) $_POST['custom_bundle_ids'] ) : array();
    ...
    public function wpb_custom_bundles_tab_data( $tabs ){
        global $product;
		$priority = WC_Admin_Settings::get_option('rhwct_tab_product_order_bundle');
        ...
        if ( !empty($tancontent)) {
            $tabs['bundles_tab'] = array(
                'title'     => $title,
                'priority'  => $priority,
                'callback'  => array( &$this, 'rhwoo_custom_bundles_tab_content' )
            );
        }
    ...
    /*  */
    public function rhwoo_custom_bundles_tab_content() {
        require RHWCT_DIRPATH .'/public/custom_bundles_tab_content.php';
    }
        ...
        if ( $product ) {
            ...
            if ( $product_type && $product_type === 'grouped' ) {
                ...
                foreach ( $product->get_children() as $child_product_id ) {
                    ...
                    if (WC()->cart->add_to_cart($child_product_id, $quantity, $variation_id, $variation, array('custom_price' => $custom_price)) && 'publish' === $product_status) {
                ...
                $is_sub_product = isset($_POST['sub_product']) && $_POST['sub_product'] === 'yes';
                $sub_product_flag = $is_sub_product ? 'yes' : 'no';
                $bundle_product_ids = empty($_POST['bundle_product_ids']) ? array() : $_POST['bundle_product_ids'];
    ...
    public function add_custom_message_to_cart_item_name( $product_name, $cart_item, $cart_item_key ) {
        ...
        foreach ( $bundle_sub_product_ids as $bundle_sub_product_id ) {
            ...
            $bundle_title = WC_Admin_Settings::get_option('rhwct_bundle_title');
            $product_name .= '<br><small style="font-size: 12px; color: red;">With this ' . $bundle_title . ' you getting ' . ( $cart_item['custom_discount'] ) . ' % discount<br>('; 
    ...
    /*  */
    public function custom_bundles_price( $main_product_id ) {
        global $product, $post;
